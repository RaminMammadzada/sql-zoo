...
FROM ( SELECT constituency, party, RANK() OVER (PARTITION BY constituency ORDER BY votes DESC) as position
       FROM ge
       WHERE constituency BETWEEN 'S14000021' AND 'S14000026'
              AND yr  = 2017
     ) x
...
FROM (
  SELECT constituency, party, RANK() OVER (PARTITION BY constituency ORDER BY votes DESC) as position
  FROM ge
